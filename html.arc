...
(def color rgb (map [max 0 (min 255 _)] rgb))

(def dehex (str) (errsafe (coerce str 'int 16)))

(defmemo hex>color (str)
  (and (is (len str) 6)
       (map dehex:string (str coerce 'cons tuples 2))))

(def hsv (col)
     (withs ((r g b) col
             x ((2 * r - g - b) / 2)
             y ((sqrt 3) / 2 * (g - b)))
       (list (if (or (x no:is 0) (y no:is 0)) (atan y x) -4)
             (sqrt (x * x + y * y))
             (/ (+ r g b) 3))))
...
(defmemo hexrep (col) (string (map hexreps col)))
