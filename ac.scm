...
(define (ac s env)
  (cond
    ...
    ((pair? s)
     (cond
       ((ssyntax? (car s)) (ac (cons (expand-ssyntax (car s)) (cdr s)) env))
       ((symbol? (car s)) (case (car s)
			    (($) (ac-$ (cadr s) env))
			    ((quote) (list 'quote (ac-niltree (cadr s))))
			    ((quasiquote) (ac-qq (cadr s) env))
			    ((if) (ac-if (cdr s) env))
			    ((fn) (ac-fn (cadr s) (cddr s) env))
			    ((assign) (ac-set (cdr s) env))
			    (else (ac-call (car s) (cdr s) env))))
       ((pair? (car s))
	; the next three clauses could be removed without changing semantics
	; ... except that they work for macros (so prob should do this for
	; every elt of s, not just the car)
	(case (car (car s))
	  ((compose) (ac (decompose (cdar s) (cdr s)) env))
	  ((complement) (ac (list 'no (cons (cadar s) (cdr s))) env))
	  ((andf) (ac-andf s env))
	  (else (ac-call (car s) (cdr s) env))))
       (else (ac-call (car s) (cdr s) env))))
    (else (err "Bad object in expression" s))))
...
(define (ssyntax-expander sym)
  (let ((name (symbol->chars sym)))
    (cond ((or (memq #\: name) (memq #\~ name)) expand-compose)
          ((or (memq #\. name) (memq #\! name)) expand-sexpr)
          ((memq #\& name) expand-and)
          ((memq #\_ name) expand-curry)
          ((memq #\% name) expand-rev-curry)
...
(define (expand-rev-curry sym)
  (cons 'ofn (map (lambda (x) 
                    (if (pair? x) (chars->value x) x))
                  (tokens (lambda (c) (eqv? c #\%)) 
                          (symbol->chars sym) 
                          '() 
...
(define (ac-call fn args env)
  (let* ((v (and (symbol? fn) (bound? fn)))
         (macfn (ac-macro? v)))
    (cond (macfn
            (ac-mac-call macfn args env))
          ((and (pair? fn) (eqv? (car fn) 'fn))
           `(,(ac fn env) ,@(ac-args (cadr fn) args env)))
          ((and (ar-bflag 'direct-calls) (not (lex? fn env)) (procedure? v))
           (ac-global-call fn args env))
          ((ar-bflag 'uniform-notation)
           `((ar-coerce ,(ac fn env) 'fn) ,@(map (lambda (x) (ac x env)) args)))
          (#t (ac* fn args env)))))

;; polynotation
(define (ac* fst rst env)
    (case (notation ac-mac/fn? fst rst env)
      ((prefix) `(,(ac fst env) ,@(map (lambda (x) (ac x env)) rst)))
      ((data-call) `((ar-coerce ,(ac fst env) 'fn) ,@(map (lambda (x) (ac x env)) rst)))
      ((needs-eval) `(ar-call-resolve-notation ,(ac fst env) ,@(map (lambda (x) (ac x env)) rst)))
      ((postfix) (ac (postfix->prefix list (cons fst rst)) env))
      ((infix) (ac (infix->prefix cons (cons fst rst)) env))
      ((unknown) (error "Unknown notation" (cons fst rst)))
      (else (error "Invalid expression" (cons fst rst)))))

(define (ac-mac/fn? env k)
  (lambda (name)
    (or (and (pair? name) (case (car name)
                            ((quote) #f)
                            ((ofn fn make-br-fn) #t)
                            (else (k 'needs-eval))))
        (and (ssyntax? name)
             (not (eq? (ssyntax-expander name) expand-sexpr)))
        (and (symbol? name)
             (if (lex? name env) (k 'needs-eval)
               (or (member (list name) env)
                   (let ((v (or (bound? name) (k 'needs-eval))))
                     (or (procedure? v)
                         (ac-macro? v)))))))))

(define ar-call-resolve-notation
  (let ((cache (make-hash-table 'equal)))
    (lambda ls
      (if (procedure? (car ls)) (apply (car ls) (cdr ls))
        (let ((expr-type (map procedure? ls)))
          (let ((fn (hash-table-get cache expr-type #f)))
            (if fn (fn ls)
              (let ((fn (eval* ls)))
                (hash-table-put! cache expr-type fn)
                (fn ls)))))))))

(define (eval* ls)
    (case (notation ar-mac/fn? (car ls) (cdr ls) #f)
      ((prefix) (lambda (ls) (apply (car ls) (cdr ls))))
      ((data-call) (lambda (ls) (apply (ar-coerce (car ls) 'fn) (cdr ls))))
      ((postfix) (lambda (ls) (postfix->prefix (lambda (fn data) (fn data)) ls)))
      ((infix) (lambda (ls) (infix->prefix apply ls)))
      ((needs-eval) (error "Not all symbols bound at run-time" ls))
      ((unknown) (error "Unknown notation" ls))
      (else (error "Invalid expression" ls))))

(define (ar-mac/fn? env k)
  (lambda (x)
    (or (procedure? x)
	(and (ac-macro? x)
	     (error "Use of macro inside parse-time opaque notation" x)))))

(define (ac-unexpanded? x)
  (and (pair? x)
       (not (eq? (car x) 'quote))))

; Notation discovery. When called at runtime, env is #f.
; - On no arguments, assume prefix notation
; - If first arg is = etc., assume infix (enables infix assignment etc.)

(define (notation tester fn args env)
  (if (null? args) 'prefix
    (if (any (lambda (token) (memq token args))
             '(of = fn ->)) 'infix
      (call-with-current-continuation
        (lambda (k)
          (let ((proc? (tester env k)))
            (if (or (proc? fn)
                    (memq fn '(quote quasiquote splice unquote))) 'prefix
              (if (every proc? args) 'postfix
                (if (proc-data-pairs proc? args) 'infix
                  (if (every (lambda (x) (not (proc? x))) args) 'data-call
                    'unknown))))))))))
...
(define (proc-data-pairs proc? ls)
  (or (null? ls)
      (and (proc? (car ls))
           (not (null? (cdr ls)))
           (not (proc? (cadr ls)))
           (proc-data-pairs proc? (cddr ls)))))

(define (postfix->prefix f ls)
  (let it ((data (car ls)) (fns (cdr ls)))
    (if (null? fns) data
      (it (f (car fns) data) (cdr fns)))))
...
(define infix->prefix
  (letrec
    ((first-order
       (lambda (n ls)
	 (if (null? (cdr ls)) n
	   (first-order (min n (precedence (cadr ls))) (cddr ls))))))
    (lambda (fn ls)
      (if (null? (cdr ls)) (car ls)
	(let lp ((fo (first-order 99 ls)) (left `(,(car ls))) (ls (cdr ls)))
	  (if (null? ls) (infix->prefix fn (reverse left))
	    (if (= fo (precedence (car ls)))
	      (lp fo (cons (fn (car ls) `(,(car left) ,(cadr ls)))
			   (cdr left))
		  (cddr ls))
	      (lp fo `(,(cadr ls) ,(car ls) ,@left) (cddr ls)))))))))
...
;;        (lambda (n ls)
;;          (if (null? (cdr ls)) n
;;            (max-order (max n (precedence (cadr ls)))  (cddr ls))))))
;;     (lambda (fn ls)
;;       (if (null? (cdr ls)) (car ls)
;;         (let loop ((n (max-order 0 ls)) (left (list (car ls))) (ls (cdr ls)))
;;           (if (= n (precedence (car ls)))
;;             (fn (car ls) (list (infix->prefix fn (reverse left))
;;                                (infix->prefix fn (cdr ls))))
;;             (loop n (cons (cadr ls) (cons (car ls) left)) (cddr ls))))))))

;; Current precedence rules set has one difference from that of C language --
;; binary operators have precedence over predicates:
;; (a shl b is c) === (is (shl a b) c)

;(define (precedence f)
;  (case (hash-table-get fn->infixoperator f '+)
;    ((expt root) 0) ((* / %) 1) ((+ -) 2) ((<< >>) 3) ((&) 4) ((^) 5)
;    ((vbar) 6) ((< <= > >=) 7) ((== !=) 8) ((&&) 9) ((dblbar) 10)
;    ((= += -= *= /= %=) 11)))

(define precedence
  (letrec ((mkls (lambda (n ls)
                   (cond ((null? ls) '())
                         ((pair? (car ls)) (append (mkls n (car ls))
                                                   (mkls (+ n 1) (cdr ls))))
                         (else (cons (list (car ls) n) (mkls n (cdr ls))))))))
    (let
      ((tbl (mkls 0 '((-> dot ::)
                      (! ~ sizeof type cast ++ --) 
                      (expt root)
                      ...
                      (= += -= *= /= %=)))))
      (lambda (f)
        (cadr (assq (hash-table-get fn->infixoperator f '+) tbl))))))

;; Infix notation generally uses limited number of predefined operators rather
;; than unlimited number of functions. To make precedence manageable, we can
;; associate our functions to those operators.

(define fn->infixoperator (make-hash-table 'equal))

;; Allow to control infix precedence rules from arc

(xdef infixop fn->infixoperator)

;; Usage: (= ^ expt) (setinfixop '^ 'expt)
(xdef setinfixop
      (lambda (sym op) 
        (hash-table-put! fn->infixoperator sym op)
        (hash-table-put! fn->infixoperator (bound? sym) op)))

(for-each (lambda (ls) (hash-table-put! fn->infixoperator (car ls) (cadr ls)))
          '((+ +)
	    (- -)
	    (* *)
	    (/ /)
...
(define (ac-macex e . once)
  (if (pair? e)
      (let ((m (and (symbol? (car e)) (ac-macro? (bound? (car e))))))
        (if m
  ...
  (syntax-rules ()
    ((xxdef a b)
     (let ((nm (ac-global-name 'a))
       ...
       (let ((op (hash-table-get fn->infixoperator 'a #f)))
         (and op (hash-table-put! fn->infixoperator a op)))
       a))))
...
(for-each (lambda (e)
 ...
 `((fn      (cons   ,(lambda (l) (lambda (i) (list-ref l i))))
            ...
            (int    ,(lambda (i) (lambda (x)
				     ...
				     ;   	   (lambda (n str)
				     ;   	     (if (< n 1) str
				     ;   	       (conc (- n 1) (string-append str x))))))
				     ;     (conc i "")))
				     ;  ((char) (make-string i x))
				     ;  (else (conc i '()))))))))
				     (conc i '()))))))
...
(define (tl)
  (let ((interactive? (terminal-port? (current-input-port))))
    (when interactive? 
      (display "\e[1mGOG: \e[34m^D\e[37m QUITS, \e[34m^C\e[37m BREAKS;")
      (display " \e[37mOUTMOST \e[31m(")
      (display "\e[37m AND \e[31m)\e[37m ARE OMITTABLE.\e[0m\n"))
    ;GOG v0.0.1 Based on the ARC language by Paul Graham and Robert Morris.
    ;Welcome to Gog! Gog is an extension of ARC written by Dmitri Nosovicki.
    (tl2 interactive?)))

(define gog tl)

(define (unclosed-paren? str)
  (let ((match (member "to" (map list->string
				 (tokens (lambda (c) (eqv? #\space c))
					 (string->list str)
					 '()
					 '()
					 #f)))))
    (and match (equal? "close" (xcar (cdr match))))))

(define (get-input interactive? str)
  (if (eof-object? str) str
    (on-err
      (lambda (c)
	(if (unclosed-paren? (exn-message c))
	  (begin (when interactive? (display "       "))
...
(define (tl2 interactive?)
  ...
  (on-err (lambda (c) 
	    ...
	    (parameterize ((current-output-port (current-error-port)))
			  ...
			  (tl2 interactive?)))
	  (lambda ()
	    (let ((input (get-input interactive? (read-line))))
	      (when (eof-object? input)
		(and interactive? (newline))
		(exit))
	      (if (null? input) (tl2 interactive?)
		(let ((expr (if (pair? (cdr input))
			      input
			      (car input))))
		  (if (eqv? expr ':a)
		    'done
    ...
    (call-with-input-file inname
      (lambda (ip)
        (call-with-output-file outname 
          (lambda (op)
            (acompile1 ip op (if (null? ck) #f (car ck)))))))))
