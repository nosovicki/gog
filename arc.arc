...
; Uses splice, otherwose xcessive apply wastes GC

(def join args
  (if (no args) nil
  `(,@(car args) ,@(apply join (cdr args)))))
...
(def isnt args (no (apply is args)))
...
(map [declaration _ decl-bool]
     '(atstrings direct-calls explicit-flush uniform-notation))
...
(def read-table ((o i (stdin)) (o eof))
  (aand (read i eof)
        (mutable (if (alist it) (listtab it)
                     it)))) ; fixed mutability by dn
...
(def unixtime (year mon day (o hour 0) (o min 0) (o sec 0))
     (if (<= mon 2) (unixtime (- year 1) (+ mon 12) day hour min sec) ;; Puts Feb last since it has leap day
       (((((year / 4 - year / 100 + year / 400 + 367 * (mon - 2) / 12 +
		 day) + year * 365 - 719499) * 24 + hour) * 60 + min) * 60 + sec)))
...
(def randseq(len (o init 0))
  (and (len > 0) (cons init (randseq (len - 1) (+ init (rand 3) -1)))))
...
;; Mzscheme reads structures from file as immutable.
;; To add mutability we deep copy everything. -- dn
...
;; begin auxiliaries for polynotational semantics
...
(def ofn (f . args)
  ...
  ;(fn lst (apply f (+ (if lst (map (fn (a) (pass (car lst) a)) args) args) lst))))
  (fn lst (apply f (+ args lst))))

(def r (f) (fn args (apply f (rev args))))
;; end transformers

;; begin Examples, i.e. tests
(declaration 'test)
(declare 'test nil)
(= examples* (table))
(mac example (sym expr)
     (if (or (no:decl 'test) (eval expr)) `(zap [cons B A] (examples* ',sym) ',expr)
	   (err sym (string "Wrong " sym " example:\n" (tostring (ppr expr))
			    (and (>expr is 'is) (string ":\n" (65 "=") "\n"
							(tostring:ppr (eval ><expr))
							"\n!=\n"
							(tostring:ppr (eval ><<expr))
							"\n" (65 "=")))))))

(def test (sym)
     (or (prall (keep no:eval (examples* sym)) (string sym ": Following tests failed:\n") "\n")
	 (prn sym ": OK")))
;; end Examples, i.e. tests

;; begin PHP goodies implementation
(def file (fp)
  (if (isa fp 'string) (file (infile fp))
    ((afn ((o str "") (o ls))
	  (if str (self (readline fp) (cons str ls))
	    (cdr (rev ls)))))))
;; end PHP goodies implementation
